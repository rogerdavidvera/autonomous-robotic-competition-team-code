/*
   Milestone 8
   Find Your Own Egg and Bring It Home
...
void loop() {
  ...
  if (rightRGBWhite() || leftRGBWhite()) {    // lineCheck() will return true if Ovi 
      Serial.println("White Line Detected");  // hits a color boundary or white line
      escapeWhiteLine();
  }
  ...
  if (findStartingQ) {              // this case is for finding the starting quadrant
    if (boundaryTrigger()) {        // boundaryTrigger() will return true is Ovi hits a color boundary
      ...
      forward();
      delay(700);                   // We need that to fully exit the loop.
    }
    else {
  ...
  */
    ...
    /* ONLY COLLECT ONE EGG FOR MILESTONE */
    if (eggsCollected >= 1) {
      stopMovement();
      delay(200);
      findHome = true;
    ...
    if (eggFound && isEggInRange()) {
      stopMovement();
      reverse(); // Back up in order to get in proper position
      delay(300);
      repositionEgg(); // Get arms ready
      scoopEgg();
      sortEgg(); // Check if egg is proper color; if so, keep in basket
      stopMovement();
      delay(2000);
      reverse(); // **Necessary if egg is too close to robot.
      delay(750);
      eggFound = false;
      eggInRange = false;
    }
