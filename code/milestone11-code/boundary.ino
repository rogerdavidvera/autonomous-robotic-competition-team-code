/*
   ...
   DETECTING WHITE LINES
      lineCheck() -- returns true if Ovi is at a white line or color boundary
...
bool leftWhiteCheck(uint16_t red, uint16_t green, uint16_t blue, uint16_t l) {
   if ((l >= 78 && l <= 400 &&
      ...
      (l >= 310 && l <= 470 &&  // Mild raised edge in corners
       ...
       green >= 650 && green <= 680 &&
       blue >= 550 && blue <= 580) ||
      (l >= 200 && l <= 270 &&  // Severe raised edge in corners
       red >= 265 && red <= 340 &&
       green >= 304 && green <= 450 &&
       blue >= 250 && blue <= 330) ||
...
bool boundaryTrigger() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    // If leftTCS doesn't find anything, try rightTCS
    //delay(20);
...
//    if (rightColorCheck(r, g, b, lux)) {                      // this will return true if the right TCS reads a color
//      Serial.println("THE RIGHT RGB SENSOR HAS DETECTED A COLOR BOUNDARY.");
//      whichSensor = right;
//      return true;
//    }
  }
